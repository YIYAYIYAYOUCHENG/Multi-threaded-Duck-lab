...
#define TEST_OUTPUT_ROOT_FILE "RootStorage_IntegrationTest_outfile.root"
...
static DataInt32 *data2;
static DataUInt8 *data3;
static DataUInt8 *data4;

static void load_data_vector(vector<const Data*> &data)
{
	int32_t init_val[] = {0, 0, 0, 0};
	int8_t init_val2[] = {0, 0, 0, 0};
	data1 = new DataInt8(data1_name, 10, NULL);
	data2 = new DataInt32(data2_name, 3, init_val);
	data3 = new DataUInt8(data3_name, 1, (unsigned char*) init_val2);
	data4 = new DataUInt8(data4_name, 4, (unsigned char*) init_val2);
	data.clear();
...
static int check_rootstorage(void)
{
	...
	for (int i = 0; i < n_event; i++) {
		...
		data2 = static_cast<DataInt32*>(data[1]);
...
int main(int argc, char* argv[])
{
	...
	TEST_LOG(" -> Storage Integration Test: __RootStorage__" << endl);
